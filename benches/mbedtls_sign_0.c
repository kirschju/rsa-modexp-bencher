...
// BENCH_FUNC mbedtls_rsa_private
//
#ifndef BENCH_FUNC
...
#error "mbedtls version not supported"
    ...
    if (BENCH_FUNC(&ctx, mbedtls_test_rnd_std_rand, NULL, msg, sig) != 0) {
        fprintf(stderr, "mbedtls_rsa_private\n");
        return BENCH_ERR_SIGN_FAILED;
    }

    if (mbedtls_rsa_public(&ctx, sig, chk) != 0) {
    ...
    *rounds = 0;
    ...
    while (!test_done) {
        BENCH_FUNC(&ctx, mbedtls_test_rnd_std_rand, NULL, msg, sig);
        *rounds += 1;
    }
