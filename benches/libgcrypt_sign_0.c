...
int bench(unsigned long long *rounds)
{
    ...
    x = gcry_mpi_new (TESTVAL_NUMBITS-16*8);
    gcry_mpi_randomize (x, TESTVAL_NUMBITS-16*8, GCRY_WEAK_RANDOM);
    ...
    err = BENCH_FUNC(&sig, data, sec_key);
    ...
    *rounds = 0;
    ...
    while (!test_done) {
        if (BENCH_FUNC(&sig, data, sec_key)) {
            perror("sign");
            return BENCH_ERR_SIGN_FAILED;
        }
        *rounds += 1;
